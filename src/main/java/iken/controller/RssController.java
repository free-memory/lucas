...
public class RssController extends RestfulController {
    ...
    public ResponseEntity<String> listArticles(ModelMap model, @RequestParam String siteId, @RequestParam int pageNo, @RequestParam int pageSize) {

        JSONObject jsonResponse = new JSONObject();
        RssService rssService = new RssServiceImpl();
        try {
            List<Article> articles = rssService.getArticles(siteId, pageNo, pageSize);
    ...
    public ResponseEntity<String> catchArticles(ModelMap model, @RequestParam(value = "siteId") String strSiteId) {
        JSONObject jsonResponse = new JSONObject();
        RssService rssService = new RssServiceImpl();

        String siteId = strSiteId.substring(strSiteId.indexOf("=") + 1);
